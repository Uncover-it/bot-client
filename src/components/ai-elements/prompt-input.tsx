...
export const PromptInput = ({
  ...
  const next: (FileUIPart & { id: string; file?: File })[] = [];
        for (const file of capped) {
          next.push({
            id: nanoid(),
            type: "file",
            // Keep a blob reference so consumers can upload the original file.
            // This is intentionally an extra field in the FileUIPart-like object.
            file,
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    const files: (FileUIPart & { file?: File })[] = items.map(({ file, ...item }) => ({
      ...item,
      file,
    }));
